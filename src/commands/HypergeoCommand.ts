...
export default class HypergeoCommand implements AbstractSlashCommand {
  ...
  constructor() {
    this.data = new SlashCommandBuilder()
      .setName('hypergeo')
      .setDescription(
        'Calculates the probability to draw a number of specific cards, given the number of copies in a deck.'
      )
  ...
  hypergeometric(N: number, K: number, n: number, k: number) {
  ...
  hypergeometricAtLeast(N: number, K: number, n: number, k: number) {
    let probability = 0;
  ...
  hypergeometricAtMost(N: number, K: number, n: number, k: number) {
    let probability = 0;
  ...
  hypergeometricZero(N: number, K: number, n: number) {
    return this.hypergeometric(N, K, n, 0);
  }

  getOptions(interaction: Interaction): number[] {
    if (!interaction.isChatInputCommand()) return [];
    const values = interaction.options.data.map(({ value }) => value as number);
  ...
  exec(interaction: Interaction): void | Promise<InteractionResponse<boolean>> {
    if (!interaction.isChatInputCommand()) return;
    ...
    if (copiesInDeck > deckSize || cardsDrawn > deckSize) {
      return interaction.reply(
        `⚠️ Copies in deck (${copiesInDeck}) and cards drawn (${cardsDrawn}) must be less than or equal to the total deck size (${deckSize}).`
      );
    }

    const exactly = this.hypergeometric(
      deckSize,
      copiesInDeck,
      cardsDrawn,
      copiesDrawn
    ...
    const atLeast = this.hypergeometricAtLeast(
      deckSize,
      copiesInDeck,
      cardsDrawn,
      copiesDrawn
    ...
    const atMost = this.hypergeometricAtMost(
      deckSize,
      copiesInDeck,
      cardsDrawn,
      copiesDrawn
    ...
    const message = [
      ...
      `Deck size: ${deckSize}`,
      `Copies in deck: ${copiesInDeck}`,
      `Cards drawn/seen: ${cardsDrawn}`,
      `Copies drawn/seen: ${copiesDrawn}\n`,

      `*Exactly* ${copiesDrawn}:\t**${exactPercentage}**`,
      `*At least* ${copiesDrawn}:\t**${atLeastPercentage}**`,
      `*At most* ${copiesDrawn}:\t**${atMostPercentage}**`,
      `*Draw 0*:\t\t**${zeroPercentage}**`,
    ];
